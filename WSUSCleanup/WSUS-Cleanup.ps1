...
if ((Test-Admin) -eq $false)  {
    if ($elevated) 
    {
        ...
        if(!(Test-Path -Path .\WsusDBMaint.sql))
        {
        Set-Location -Path "C:\WSUSCleanup"
        }
    } 
    else {
...
#Local Internal SQL Server: \\.\pipe\Microsoft##WID\tsql\query
...
function RebuildDBIndexes{
    ...
    try{
        Import-Module UpdateServices
        $status = SQLCMD -S $SqlServer -i $SQLPath -I 
